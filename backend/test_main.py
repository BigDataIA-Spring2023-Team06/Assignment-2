...
def test_copy_to_s3_goes():
    response = client.post("/copy_to_s3/")
    ...
    response_s3 = requests.post("/copy_to_s3/", params=payload)
...
def test_get_files_goes_api():
    response = client.get("/get_files_goes/2022/09/09/09")
...
def test_get_url_goes_original():
    response = client.get("get_url_goes_original/OR_ABI-L1b-RadC-M6C01_G18_s20222961901174_e20222961903547_c20222961903582.nc")
    assert response.status_code == 200
...
def test_get_files_noaa_api():
    response = client.get("get_files_noaa/KABR/2022/09/17/05")
    assert response.status_code == 200
...
def test_get_url_nexrad_original():
    response = client.get("get_url_nexrad_original/KABR20220917_050243_V06")
    assert response.status_code == 200
    url = response.json()['original url']
...
def test_plot_map_viz():
    response = client.get("/map_visualization/KABR")
    assert response.status_code == 200
